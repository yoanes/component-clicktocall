...
When /^the "([^"]+?)" page should be displayed$/ do |page_name|
  ...
  begin
    @page_model = Pages.const_get(page_model_class_name).new(@device, page)
  rescue
...
When /^the "([^"]+?)" page should be displayed for "([^"]+?)"$/ do |page_name, logical_data_item|
  page_model_class_name = page_name + "PageModel"
  begin
    @page_model = Pages.const_get(page_model_class_name).new(@device, page, @data[logical_data_item])
  rescue
    raise $!, "Could not instantiate class with name of #{page_model_class_name}." , $!.backtrace
  end
...
When /^I click .* link for "([^"]+?)"$/ do |logical_data_item|
  click_link @data[logical_data_item].link_text
end
...
When /^the system should( not)? log .* for "([^"]+?)"$/ do |boolean, logical_data_item|
  expected = !boolean
  if (expected)
    @log_deltas.should have_unique(@data[logical_data_item].log_string)
  else
    @log_deltas.should_not have_unique(@data[logical_data_item].log_string)
  end
end
